...
| matched_industry_specify_others | `x_studio_specify_others` from the same order `customer_industry_proxy.csv` used to pick `matched_industry` - only meaningful when `matched_industry` is literally `"Others"` (that's the only value Odoo's Studio config populates this free-text field for); blank otherwise. Carried through as its own column, never folded into `matched_industry` itself |
...
| matched_order_id, matched_rfq_number, matched_order_industry, matched_order_industry_specify_others, matched_order_customer_type, matched_order_region, matched_order_po_number, matched_order_po_value, matched_order_quote_status, matched_order_date | Only populated on an `RFQ_MATCH` row - the **specific matched `sale.order`'s own** fields (not the customer-level aggregation above), resolved by exact-matching `quotation_number` against `sale_orders.csv`'s `x_studio_internal_rfq_assignment_number` column. Blank for every other match status. `matched_order_industry_specify_others` is `x_studio_specify_others` straight off that same order - same "Others"-only caveat as `matched_industry_specify_others` above. |
...
| product_family | For "commonly quoted modules" analysis. `model_canonical` checked first against `model_family_rules.csv` (precise, prefix match), else `description` checked against `description_family_rules.csv` (broader coverage, first-match-wins phrase rules) - see `knowledge_bank/product_family.py`. Blank when neither matches - **never a guess**. Coverage is inherently partial: 19.5% of `ATTACHMENT_ITEM` rows with the starter tables (2026-09-18), since 68.3% of rows have no `description` in the first place and only 5.5% have a recognized `model_canonical` |
| product_family_basis | `MODEL_RULE` / `DESCRIPTION_RULE` / blank, as for `make_canonical_basis` |
...
| matched_order_id, matched_rfq_number, matched_order_industry, matched_order_industry_specify_others, matched_order_po_number, matched_order_po_value, matched_order_quote_status, order_state, firm_or_budgetary | The specific Odoo order's own fields, where one was matched |
...
## model_family_rules.csv / description_family_rules.csv (knowledge_bank/)

Human-curated rule tables in `Quotation_Data/07_knowledge_bank/`
(gitignored), read by both knowledge-bank builders via
`knowledge_bank/product_family.py`. Missing file = empty table (no
`product_family` values, not an error). Unlike the alias tables above,
there's no algorithmic signal to propose a family name from scratch -
these are edited directly, not generated by a suggester and reviewed.

| Field | Description |
|---|---|
| pattern | `model_family_rules.csv`: matched against `model_canonical` by **prefix** (`ULTRAMAT` matches `ULTRAMAT 23`, `ULTRAMAT 6E`). `description_family_rules.csv`: a case-insensitive regex `re.search`'d anywhere in `description` - a plain phrase works as-is (`GAS CHROMATOGRAPH`), alternation covers spelling variants (`OXYGEN ANALY(S\|Z)ER`) |
| family | The value written to `product_family` (stored uppercase) |
| note | Free text |

**Row order matters for `description_family_rules.csv`** - rules are
checked top-to-bottom and the first match wins, so a more specific rule
(`OXYGEN ANALYSER`) must be listed before a more generic one that could
also match the same text (`GAS ANALYSER`) whenever the two overlap.
Model rules are checked before description rules for a given row
(`model_canonical` is the more precise signal when it exists).

**The shipped starter tables are drafts, not verified ground truth** -
the family names and model→family mappings were drawn from general
product-line knowledge, not confirmed against anything in this repo, and
need your domain review before being trusted (same caution the alias
tables needed for make/model). `knowledge_bank/suggest_product_families.py`
reports the highest-frequency still-uncategorized `model_canonical`
values and description phrases, to help extend the tables.
